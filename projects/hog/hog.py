...
def boar_brawl(player_score, opponent_score):
    ...
    one_score = player_score % 10
    ten_score = opponent_score % 100 // 10
    abs_score = max(one_score - ten_score, ten_score - one_score)
    return max(abs_score * 3, 1)
    # END PROBLEM 2
...
def is_always_roll(strategy, goal=GOAL):
    ...
    "*** YOUR CODE HERE ***"
    i = 0
    start_num_roll = strategy(0, 0)
    while i < goal:
        j = 0
        while j < goal:
            if strategy(i, j) != start_num_roll:
                return False
            j = j + 1
        i = i + 1
    return True
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    count_num_rolls = 0
    while num <= 10:
        current_average = make_averaged(roll_dice, samples_count)(num, dice)
        if average < current_average:
            average = current_average
            count_num_rolls = num
        num += 1
    return count_num_rolls
    # END PROBLEM 9
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    boar_score = boar_brawl(score, opponent_score)
    boal_sus_point = sus_points(score + boar_score) - score
    if boal_sus_point >= threshold:
        return 0
    else:
        return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11
