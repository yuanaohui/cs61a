...
class Ant(Insect):
    ...
    def double(self):
        ...
        if self.is_container:
            if not self.is_double and self.damage:
                self.damage = self.damage *2
                self.is_double = True
            if self.ant_contained and not self.ant_contained.is_double and self.ant_contained.damage:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        look_place = self.place
        count = 0
        while not look_place.is_hive and count <= self.upper_bound:
            if not look_place.bees or count < self.lower_bound: # [] is []  False
                look_place = look_place.entrance
            else:
                return random_bee(look_place.bees)
            count += 1
        return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health <= amount:
            damage = self.health + self.damage
        else:
            damage = amount
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.turns_to_chew:
            ...
            chose_bee = random_bee(self.place.bees)
            chose_bee.reduce_health(chose_bee.health)
...
class ContainerAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.ant_contained != None:
            self.store_ant(self.ant_contained)
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        look_place = self.place.exit
        while look_place:
            if look_place.ant:
                look_place.ant.double()
            look_place = look_place.exit
        # END Problem 12
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        if self.place.ant.is_container:
            a[self.place.ant] = distance
        while look_place:
            if look_place.ant and look_place != self.place:
                a[look_place.ant] = distance
            if look_place.bees:
                for i in look_place.bees:
                    a[i] = distance
            distance += 1
            look_place = look_place.entrance
        return a
