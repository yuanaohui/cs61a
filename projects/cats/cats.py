...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    ps_of_s = [x for x in paragraphs if select(x)]
    if len(ps_of_s) <= k:
        return ''
    else:
        return ps_of_s[k]
...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def select(x):
        list_x = split(lower(remove_punctuation(x)))
        count = 0
        while count < len(list_x):
            if list_x[count] in subject:
                return True
            else:
                count += 1
        return False
...
def accuracy(typed, source):
    ...
    count = 0
    score = 0
    while count < min(len(source_words), len(typed_words)):
...
def feline_fixes(typed, source, limit):
    ...
    if typed == "" and source == "":
        return 0
    if typed == "" or source == "" or typed[0] != source[0]:
        if limit == 0:
            return 1
...
def minimum_mewtations(typed, source, limit):
    ...
    if  typed == '' or source == '' or typed[0] == source[0]: # Feel free to remove or add additional cases
        # BEGIN
        "*** YOUR CODE HERE ***"
        if typed == '':
            if limit == 0:
...
def report_progress(typed, source, user_id, upload):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    def fun_progress(typed, source):     
        if typed == [] or typed[0] != source[0]:
            return 0
        else:
            return fun_progress(typed[1:], source[1:]) + 1
    len_source = len(source)
    progress = fun_progress(typed, source) / len_source
...
def time_per_word(words, timestamps_per_player):
    ...
    for i in range(len(timestamps_per_player)):
        element = [timestamps_per_player[i][j + 1] - timestamps_per_player[i][j] for j in range(len(timestamps_per_player[0]) - 1)]
        times = times + [element]
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    memo_word = {i:[] for i in get_all_words(match)}

    for i in word_indices:
        min_time = time(match, 0, i)
        index = 0
        for j in player_indices:
            if min_time > time(match, j, i):
                min_time = time(match, j, i)
                index = j
        memo_word[get_word(match, i)] = index
